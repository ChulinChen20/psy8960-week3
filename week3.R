...
clean_df <- raw_df[raw_df$timeStart > "2017-07-01 00:00:00",] # raw_df[!grepl("2017-06", raw_df$timeStart),]
clean_df <- clean_df[clean_df$q6 == 1,]

# Analysis
clean_df$timeSpent <- difftime(clean_df$timeEnd, clean_df$timeStart,units = "secs") # clean_df$timeSpent <- as.numeric(clean_df$timeEnd - clean_df$timeStart) * 60
